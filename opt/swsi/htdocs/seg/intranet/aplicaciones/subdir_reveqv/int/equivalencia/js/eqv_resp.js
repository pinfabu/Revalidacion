$(document).ready(function() {
	...
	function validEscalas(){
		...
		if(!checkRango()){
			...
			$("#modMessage_msg").html("Debe indicar las escalas máximas y minímas de las calificaciones");
			$("#modMessage").dialog("open");
			return false;
		...
		if(!numMatFlag){
			$("#modMessage_msg").html("Debe indicar el número de materias");
			$("#modMessage").dialog("open");
			return false;
		}
		...
		$.each($(".xMat"), function(){
			...
			if(!regexp.test($(this).val())){
				$("#modMessage_msg").html("Debe indicar la calificación "+$(this).attr("xMat"));
				$("#modMessage").dialog("open");
				xError = true;
			}
	...
	function calcularPromedio(){
		...
		if(xFlag){
			xPromNal = parseFloat(parseFloat(xPromNal)/numMat).toFixed(2);
			xPromExt = parseFloat(parseFloat(xPromExt)/numMat).toFixed(2);
	...
	$(document).on('keypress','.xMat',function(event){
		...
		if(parseFloat(extMin)<parseFloat(extMax)){
			...
			if(parseFloat(calif)>parseFloat(extMax)){
				error_rango = true;
				aux_num = nowCell;
				$("#modMessage_msg").html("La calificación no puede ser mayor a la calificación máxima");
				$("#modMessage").dialog("open");
				return false;
			}
			...
			if(parseFloat(calif)<parseFloat(extMax)){
				error_rango = true;
				aux_num = nowCell; 
				$("#modMessage_msg").html("La calificación no puede ser menor a la calificación máxima");
				$("#modMessage").dialog("open");
				return false;
			}
